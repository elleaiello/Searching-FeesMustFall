...
with open('stream_feesmustfall_part2.json', 'r', encoding='utf-8') as f: #did this for stream_feesmustfall_part?.json, ? = part 1 and 2
    data_csv = []
    for line in f:
        tweet = json.loads(line)
        if 'text' in tweet:                       
            source = tweet['source']
            data_csv.append(source)

#convert array to string
data_string = "\n".join(data_csv)

# Save array data (converted into string) in file
file = open("device_csv_2.csv", "w", encoding='utf-8') #change between 1 and 2 when appropriate
